...
interface BomVersion { id: number; version_number: string; status: string; notes: string | null }
interface BomLine {
...
const UPH_EMPTY = {
  line_order: 0, section: 'ที่นั่ง', upholster_type: 'ผ้า',
...
export default function BomBuilderPage() {
  ...
  // Group material lines by Material Group
  ...
  const openAddUph = () => {
    setEditingUph(null)
    setUphForm({ ...UPH_EMPTY, line_order: sortedLines.length + 1 })
    setError(''); setUphOpen(true)
  }
  const openEditUph = (line: BomLine) => {
    setEditingUph(line)
    setUphForm({
      line_order: line.line_order, section: line.section ?? 'ที่นั่ง',
      upholster_type: line.upholster_type ?? 'ผ้า',
  ...
  const handleUphSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        line_order: uphForm.line_order,
        section: uphForm.section,
        upholster_type: uphForm.upholster_type,
        unit: uphForm.unit || null,
        note: uphForm.note || null,
      }
      if (uphForm.qty_base) {
        payload.qty_base = parseFloat(uphForm.qty_base)
        if (uphForm.qty_width_step) payload.qty_width_step = parseFloat(uphForm.qty_width_step)
        if (uphForm.qty_step_increment) payload.qty_step_increment = parseFloat(uphForm.qty_step_increment)
      } else if (uphForm.quantity_fixed) {
        payload.quantity_fixed = parseFloat(uphForm.quantity_fixed)
      }
      if (editingUph) await api.put(`/products/bom/lines/${editingUph.id}`, payload)
      else await api.post(`/products/bom/versions/${bom!.version.id}/lines`, payload)
  ...
  // ── Material edit ──────────────────────────────────────────────────────────
  const openEditMat = (line: BomLine) => {
    setEditingMat(line)
  ...
  const handleMatEditSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setSaving(true); setError('')
    try {
      await api.put(`/products/bom/lines/${editingMat!.id}`, {
        quantity_fixed: parseFloat(matEditForm.quantity_fixed),
        unit: matEditForm.unit || null, note: matEditForm.note || null,
      })
  ...
  const handleCopyBom = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCopyOpen(false)
      setVersions(await api.get<BomVersion[]>(`/products/${productId}/bom/versions`))
      await loadBom()
    } catch (err: unknown) { setError((err as Error).message) }
    finally { setSaving(false) }
  }
  ...
  // ── Line row renderer ──────────────────────────────────────────────────────
  const LineRow = ({ line, idx }: { line: BomLine; idx: number }) => {
      ...
      <div className="flex items-start gap-3 px-4 py-2.5">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm text-gray-800">
            ...
            {isUph && line.upholster_type && (
              <span className="ml-2 text-xs text-gray-500 font-medium">({line.upholster_type})</span>
            )}
          </p>
          <p className="text-xs text-gray-400 mt-0.5">
            {!isUph && line.quantity_fixed && `Qty: ${line.quantity_fixed} ${line.unit ?? ''}`}
            {isUph && line.qty_base && `Linear Step: base=${line.qty_base} / ${line.qty_width_step}cm / +${line.qty_step_increment}`}
            {isUph && !line.qty_base && line.quantity_fixed && `Fixed: ${line.quantity_fixed} ${line.unit ?? ''}`}
            {line.note && <span className="ml-2 italic">{line.note}</span>}
          </p>
        </div>
        {isDraft && (
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="ml-9 mb-5">
        ...
        {product && formatSize(product) && (
          <p className="text-xs text-gray-400 font-mono mt-0.5">Standard Size: {formatSize(product)}</p>
        )}
      </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {bom && isDraft && (
            <>
              ...
              <button onClick={handleActivate} disabled={activating || totalLines === 0} className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-green-600 hover:bg-green-700 text-white rounded-lg disabled:opacity-40"><CheckCircle size={13} /> Activate</button>
            </>
          )}
      ...
      {bom && (
        <>
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-medium text-gray-700">BOM Lines <span className="text-gray-400 font-normal">({totalLines})</span></p>
            {isDraft && (
          ...
          {totalLines === 0 ? (
            ...
            <div className="space-y-3">
              {/* Material lines grouped by Material Group */}
              {groupedMaterials.map(({ group, lines }) => (
                <div key={group?.id ?? -1} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                  ...
                  <div className="divide-y divide-gray-50">
                    {lines.map((line, i) => (
                      <LineRow key={line.id} line={line} idx={sortedLines.indexOf(line)} />
                    ))}
                  </div>
                </div>
              ))}

              {/* Upholster Placeholder lines */}
      ...
      {/* Upholster Placeholder Dialog */}
      <FormDialog open={uphOpen} onClose={() => setUphOpen(false)}
        title={editingUph ? 'Edit Upholster Placeholder' : 'Add Upholster Placeholder'}
        onSubmit={handleUphSubmit} saving={saving} error={error} width="max-w-lg">

        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label required>Section (ส่วนของโซฟา)</Label>
            <Select required value={uphForm.section} onChange={e => setUphForm(f => ({ ...f, section: e.target.value }))}>
              {SECTIONS.map(s => <option key={s} value={s}>{s}</option>)}
            </Select>
          </div>
          <div>
            <Label required>ประเภทผ้า/หนัง</Label>
        ...
        {/* Fixed Qty first (primary) */}
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 space-y-2">
          <p className="text-xs font-medium text-gray-600">Quantity</p>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>Fixed Qty</Label>
              <Input type="number" step="0.0001" disabled={!!uphForm.qty_base}
                value={uphForm.qty_base ? '' : uphForm.quantity_fixed}
                onChange={e => setUphForm(f => ({ ...f, quantity_fixed: e.target.value }))} placeholder="1.5" />
            </div>
            <div>
              <Label>Unit</Label>
              <Input value={uphForm.unit} onChange={e => setUphForm(f => ({ ...f, unit: e.target.value }))} placeholder="เมตร" />
            </div>
          </div>
        </div>

        {/* Linear Step (Phase 2 — collapsed) */}
        <details className="rounded-lg border border-gray-200">
          <summary className="px-3 py-2 text-xs font-medium text-gray-400 cursor-pointer select-none hover:text-gray-600">
            Linear Step Formula (Phase 2)
          </summary>
          <div className="px-3 pb-3 pt-2 grid grid-cols-3 gap-2 bg-gray-50/50">
            ...
            <div><Label>Width step (cm)</Label><Input type="number" step="0.01" value={uphForm.qty_width_step} onChange={e => setUphForm(f => ({ ...f, qty_width_step: e.target.value }))} placeholder="10" /></div>
        ...
        <div>
          <Label>ลำดับใน BOM</Label>
          <Input type="number" value={uphForm.line_order} onChange={e => setUphForm(f => ({ ...f, line_order: +e.target.value }))} />
        </div>
        <div><Label>Note</Label><Input value={uphForm.note} onChange={e => setUphForm(f => ({ ...f, note: e.target.value }))} /></div>
      </FormDialog>

      {/* Material Edit Dialog */}
      <FormDialog open={matEditOpen} onClose={() => setMatEditOpen(false)} title="Edit Material Line"
        onSubmit={handleMatEditSubmit} saving={saving} error={error}>
